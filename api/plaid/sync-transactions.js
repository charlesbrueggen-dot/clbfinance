...
// Fetches latest transactions for all connected items and upserts into account_transactions
// POST { userId }  →  { synced: N, removed: N }
...
module.exports = async function handler(req, res) {
  ...
  try {
    // 1. Get all connected Plaid items for this user
    ...
    for (const item of items) {
      ...
      while (hasMore) {
        // 2. Use Plaid /transactions/sync (incremental)
        ...
        // 3. Resolve account_id for each plaid account_id
        const { data: accts } = await supabase
          .from('accounts')
          .select('id, plaid_account_id, balance')
          .eq('user_id', userId)

        const acctMap = {}
        for (const a of (accts || [])) acctMap[a.plaid_account_id] = a

        // 4. Upsert added + modified
        const toUpsert = [...added, ...modified].map(t => {
          const acct    = acctMap[t.account_id]
          const amount  = Math.abs(t.amount)  // Plaid amounts: positive = debit (expense)
        ...
        if (toUpsert.length > 0) {
          const { error: upsertErr } = await supabase
            ...
            .upsert(toUpsert, { onConflict: 'plaid_txn_id', ignoreDuplicates: false })
        ...
        for (const r of removed) {
          await supabase
            .from('account_transactions')
            .delete()
            .eq('plaid_txn_id', r.transaction_id)
          totalRemoved++
        }

        // 6. Update balances from Plaid's latest account data
        ...
        for (const pa of balRes.data.accounts) {
          ...
          if (acct) {
            await supabase
              .from('accounts')
              .update({ balance: pa.balances.current ?? acct.balance })
              .eq('id', acct.id)
              .eq('user_id', userId)
          }
        }
      ...
      // 7. Save updated cursor + last_synced_at
      await supabase
        .from('plaid_items')
...
// ── Classify transaction as expense | income | transfer ────────────────────
function classifyKind(t) {
  const cats = (t.personal_finance_category?.primary || t.category?.[0] || '').toLowerCase()
  const name  = (t.name || '').toLowerCase()

  // Plaid marks income with negative amount on their side
  if (t.amount < 0) return 'income'  // negative = money in

  if (cats.includes('transfer') || name.includes('transfer') || name.includes('zelle') || name.includes('venmo')) return 'transfer'
  return 'expense'
}

// ── Map Plaid categories → our Needs / Wants / Savings system ─────────────
function classifyCategory(t, kind) {
  ...
  if (plaidCat === 'FOOD_AND_DRINK' && (name.includes('grocery') || name.includes('whole foods') || name.includes('kroger') || name.includes('aldi') || name.includes('trader joe'))) return { category: 'Needs', subcategory: 'Groceries' }
