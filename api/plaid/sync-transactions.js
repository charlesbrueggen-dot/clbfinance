      ...
      while (hasMore) {
        ...
        const toUpsert = [...added, ...modified].map(t => {
          ...
          return {
            ...
            pending: t.pending || false, auto_categorized: true, source_type: 'plaid',
          }
        })
...
function classifyCategory(t, kind) {
  ...
  if (['RENT_AND_UTILITIES','HOME_IMPROVEMENT'].includes(plaidCat) || name.includes('rent') || name.includes('electric')) return { category: 'Needs', subcategory: 'Utilities' }
  if (plaidCat === 'FOOD_AND_DRINK' && (name.includes('grocery') || name.includes('kroger') || name.includes('whole foods'))) return { category: 'Needs', subcategory: 'Groceries' }
  if (['TRANSPORTATION'].includes(plaidCat) || name.includes('uber') || name.includes('lyft')) return { category: 'Needs', subcategory: 'Transportation' }
  if (['MEDICAL'].includes(plaidCat) || name.includes('pharmacy') || name.includes('doctor')) return { category: 'Needs', subcategory: 'Healthcare' }
  ...
  if (name.includes('netflix') || name.includes('spotify') || name.includes('hulu')) return { category: 'Wants', subcategory: 'Subscriptions' }
